...
{
  "ConnectionStrings": {
    "LocalhostConnection": "Host=localhost;Database=mauvestep;Username=<user>;Password=<password>"
  },
  "Jwt": {
    "Key": "<your-secret-key>",
    "Issuer": "<your-issuer>",
...
In `API/Program.cs`, switch the active connection string to `LocalhostConnection` for local development.
...
The API will start on `https://localhost:<port>`. The interactive Swagger UI is available at `/swagger` when running in the Development environment.

### Run with Docker

```bash
docker build -t mauvestep-backend .
docker run -e PORT=8080 -p 8080:8080 mauvestep-backend
```
